# Source : https://github.com/flyyufelix/DenseNet-Keras/blob/master/densenet121.py
from keras.models import Model
from keras.layers import Input, merge, ZeroPadding2D, Conv2D,concatenate
...
def DenseNet(nb_dense_block=3, growth_rate=32, nb_filter=64, reduction=0.0, dropout_rate=0.2, weight_decay=1e-4, classes=1000, weights_path=None):
    ...
    nb_filter = 32
    nb_layers = [3,6,8] # For DenseNet-121

    # Initial convolution
    x = ZeroPadding2D((3, 3), name='conv1_zeropadding')(img_input)
    x = BatchNormalization(epsilon=eps, axis=3, name='conv1_bn')(x)
    x = Conv2D(nb_filter, (7, 7), strides=(2, 2),activation = 'relu', name='conv1', use_bias=False)(x)
    ...
    model = Model(img_input, [lb1,lb2], name='densenet')
